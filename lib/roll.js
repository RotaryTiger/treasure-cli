...
module.exports = {
	...
	generateTreasure: function(challenge, type) {
		if (type === 'individual') {
			if (0 <= challenge && challenge <= 4) {
				return rollTable(individual_a);
			}
			else if (5 <= challenge && challenge <= 10) {
				return rollTable(individual_b);
			}
			else if (11 <= challenge && challenge <= 16) {
				return rollTable(individual_c);
			}
			else if (17 <= challenge) {
				return rollTable(individual_d);
			}
			else {
			...
			if (0 <= challenge && challenge <= 4) {
				return rollTable(hoard_a);
			}
			else if (5 <= challenge && challenge <= 10) {
				return rollTable(hoard_b);
			}
			else if (11 <= challenge && challenge <= 16) {
				return rollTable(hoard_c);
			}
			else if (17 <= challenge) {
				return rollTable(hoard_d);
			}
			else {
				console.log('"' + challenge + '" did not match any known tables. Please try again.');
			}
