...
function rollMoney(table) {
	log('rolling money for table ' + JSON.stringify(table, null, 4));
	var moneyTypes = _.keys(table.money);
	var message = '';

	_.each(moneyTypes, function(type, index) {
		var money = table.money[type];
		log('money: ' + JSON.stringify(money, null, 4));
		var amount = 0;

		if (money.roll) {
			log('rolling ' + money.roll + ' and adding it to the total');
			amount += roller.rollDice(money.roll);
		}

		if (money.value && money.value > 1) {
			log('multiplying roll by ' + money.value);
			amount *= money.value;
		}
...
function rollObjects(table) {
	...
	_.each(objectTypes, function(object, index) {
		...
		if (object.roll && object.value) {
			...
			log('did not have roll and value');
		}
	});
...
function rollTreasure(table) {
	// log('[roll treasure] table: ' + JSON.stringify(table, null, 4));
	...
	log('[roll treasure] money: ' + money);
	log('[roll treasure] objects: ' + objects);
	log('[roll treasure] magic items: ' + magicItems);
